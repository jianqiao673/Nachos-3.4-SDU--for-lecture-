...
bool FileHeader::Allocate(BitMap *freeMap, int fileSize, int incrementBytes)
{
    ...
    for ( int i = numSectors; i < numSectors + moreSectors; i++ )  
        dataSectors[i] =  freeMap->Find();  // 填入物理扇区号
...
char *FileHeader::getDataSectors()
{
    char *str = new char[NumDirector*6];  //每个扇区号占6个字符（5位数字+1个空格） 
    str[0] = '\0'; 
    for (int i=0; i<numSectors; i++) 
    { 
        char sectorStr[6]; 
        sprintf(sectorStr, "%d ", dataSectors[i]); 
        strcat(str, sectorStr); 
    } 
    return str;
}